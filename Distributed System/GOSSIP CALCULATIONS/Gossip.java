...
public class GossipStarter {
  ...
  public static void main(String[] args) throws Exception {
    System.out.println
      ("Rohan Dhoyda's Gossip Server 2.0 starting up, listening to the port " + GossipStarter.serverPort + ".\n");
    ...
    try{
      DatagramSocket DGSocket = new DatagramSocket(GossipStarter.serverPort);//A DGSocket object is created in the code which binds with GossipStarter.ServerPort using Datagram Socket Constructor
...
class ConsoleLooper implements Runnable {
  ...
  public void run(){ 
	...
	try{
	  ...
	  DatagramPacket sendPacket = new DatagramPacket(data, data.length, IPAddress, GossipStarter.serverPort); // A sendPacket wich is a new DatagramPacket is created using data, data.lengthIPAddress, GossipStarter.serverPort
